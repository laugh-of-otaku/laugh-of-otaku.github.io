...
研究ノート・日記・Aboutの本文でLaTeX形式の数式を使えます。このサイトでは、文中も独立した行も `$$` で囲みます。
...
独立した数式の前後には空行を入れます。コードとして見せたい場合だけバッククォートで囲んでください。`$...$` ではなく `$$...$$` を使います。

[数式の書き方と表示例](https://laugh-of-otaku.github.io/math/)には分数・積分・行列の例を掲載しています。数式を含むページだけMathJaxを読み込みます。表示にはJavaScriptと数式ライブラリ配信元への接続が必要です。LaTeXの文書全体や任意のパッケージを実行する機能ではありません。
...
- スタイル: `style.css`
- 数式: kramdown + MathJax 4.0.0（数式ページのみjsDelivrから読み込み）
